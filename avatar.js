...
const AvatarSystem = (() => {
  ...
  // ── NPC AVATARS ──
  function spawnNPC(x, z, shapeIdx, profile = {}) {
    ...
    const npcName = profile.name || `NPC_${String(npcs.length).padStart(2,'0')}`;
    ctx2.fillText(npcName, 64, 22);
    ...
    npcGroup.userData = {
      ...
      tag,
      npcId: profile.id || `npc_${npcs.length}`,
      name: profile.name || npcName,
      role: profile.role || 'wanderer',
      worldId: profile.worldId ?? 0,
      bio: profile.bio || 'A wandering metaverse citizen.',
